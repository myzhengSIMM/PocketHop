...
class TRANSFORMER_GCNNet(torch.nn.Module):
    def __init__(self, num_features_xd=32, n_output=2, num_features_xt=32,
                     n_filters=32, embed_dim=128, output_dim=128, hid_dim=128, dropout=None):
        
        super(TRANSFORMER_GCNNet, self).__init__()

        ## graph attention networks layers
        self.gcn1 = GCNConv(num_features_xd, num_features_xd*2)
        self.gcn2 = GCNConv(num_features_xd*2, hid_dim)
        ...
        self.out = nn.Linear(hid_dim//2, n_output)
